...
class acp_ads
{
	function make_grouplist($id)
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			$cat_option .= (in_array($row['group_id'], $id)) ? '<option selected="selected" value="' . $row['group_id'] . '">' . $group_name . '</option>' : '<option value="' . $row['group_id'] . '">' . $group_name . '</option>';
		}
	...
	function make_ranklist($id)
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			$cat_option1 = (in_array($row['rank_id'], $id)) ? '<option selected="selected" value="0">' . $user->lang['NO_RANK'] . '</option>' : '<option value="0">' . $user->lang['NO_RANK'] . '</option>';
			$cat_option .= (in_array($row['rank_id'], $id)) ? '<option selected="selected" value="' . $row['rank_id'] . '">' . $row['rank_title'] . '</option>' : '<option value="' .$row['rank_id']. '">' . $row['rank_title'] . '</option>';
		}
		$db->sql_freeresult($result);
	...
	function option_list($start, $end, $selected = 0)
	{
		...
		for($i = $start; $i <= $end; $i++)
		{
			$option .= ($i == $selected) ? '<option selected="selected" >' . sprintf ("%02d", $i) . '</option>' : '<option>' .  sprintf ("%02d", $i) . '</option>';
		}
		return $option;
	}
	...
	function main($id, $mode)
	{
		...
		$action			= ((isset($_POST['submit']) && !$_POST['ad_id']) ? 'add' : $action );
		...
		switch ($action)
		{
			...
			case 'edit':
				...
				$template->assign_vars(array(
					'POST_POSITION_1' 		=> (($row['position'] == '1') ? 'selected="selected"' : '' ),
					'POST_POSITION_2'		=> (($row['position'] == '2') ? 'selected="selected"' : '' ),
					'POST_POSITION_3'		=> (($row['position'] == '3') ? 'selected="selected"' : '' ),
					'POST_POSITION_4'		=> (($row['position'] == '4') ? 'selected="selected"' : '' ),
					'POST_POSITION_5'		=> (($row['position'] == '5') ? 'selected="selected"' : '' ),
					'POST_POSITION_6'		=> (($row['position'] == '6') ? 'selected="selected"' : '' ),
					'POST_SHOW_ALL_FORUMS'	=> (($row['show_all_forums'] == '1') ? 'checked="checked"' : '' ),
					'POST_NAME'  			=> $row['name'],
		...
		while ($row = $db->sql_fetchrow($result))
		{
			$template->assign_block_vars('ad_in_system', array(
				'U_BANNEREDIT'	=> $this->u_action . '&amp;action=edit&amp;ad_id=' . $row['ad_id'],
				'U_BANNERDEL'	=> $this->u_action . '&amp;action=delete&amp;ad_id=' . $row['ad_id'],
				'NAME'			=> $row['name'],
				'CLICKS'		=> $row['clicks'],
